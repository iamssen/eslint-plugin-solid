...
이 저장소는 과거에 브라우저 환경에서 동작하는 ESLint 번들인 `eslint-solid-standalone` 패키지를 제공했습니다. 현재는 프로젝트 복잡성을 낮추기 위해 해당 패키지는 공식적으로 유지보수하지 않으며, 단일 패키지(`@iamssen/eslint-plugin-solid`)에 집중하고 있습니다.
...
`@iamssen/eslint-plugin-solid` 및 ESLint 코어를 브라우저에서 사용하려면 Node.js 의존성(`fs`, `path`, `crypto` 등)을 Mocking(다이폴리필)하는 과정이 필요합니다.
...
export default {
  input: 'node_modules/@iamssen/eslint-plugin-solid/dist/index.js',
  output: {
