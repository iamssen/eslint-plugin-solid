...
test('flat config has meta', () => {
  expect(recommendedConfig.plugins['@ssen/solid'].meta.name).toBe(
    '@ssen/eslint-plugin-solid',
  );
  expect(recommendedConfig.plugins['@ssen/solid'].meta.version).toEqual(
    expect.any(String),
  );
  expect(typescriptConfig.plugins['@ssen/solid'].meta.name).toBe(
    '@ssen/eslint-plugin-solid',
  );
  expect(typescriptConfig.plugins['@ssen/solid'].meta.version).toEqual(
    expect.any(String),
  );
});
