 ...
 * @link https://github.com/iamssen/eslint-plugin-solid/blob/main/src/rules/reactivity/readme.md
 */
...
import { findVariable, getSourceCode } from '../../compat.js';
import type { FunctionNode, ProgramOrFunctionNode } from '../../utils.js';
import {
  findInScope,
  findParent,
  getFunctionName,
...
export default createRule<Options, MessageIds>({
  meta: {
    ...
    docs: {
      ...
      url: 'https://github.com/iamssen/eslint-plugin-solid/blob/main/src/rules/reactivity/readme.md',
    },
    schema: [
      {
        type: 'object',
  ...
  create(context, [options]) {
    ...
    const handleTrackedScopes = (
      ...
      if (
        currentScope().trackedScopes.every(
          (trackedScope) => !matchTrackedScope(trackedScope, identifier),
        )
      ) {
        const matchedExpression = currentScope().trackedScopes.some(
          (trackedScope) =>
            matchTrackedScope(
              ...
              if (arg1) {
                ...
                if (owner) {
                  ...
                  if (
                    ...
                    if (
                      (scopeStackIndex >= 1 &&
                        scopeStack[scopeStackIndex - 1].trackedScopes.every(
                          (trackedScope) =>
                            !(
                              trackedScope.expect === 'function' &&
