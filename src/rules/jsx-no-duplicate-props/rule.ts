...
export default createRule<Options, MessageIds>({
  meta: {
    ...
    docs: {
      ...
      url: 'https://github.com/iamssen/eslint-plugin-solid/blob/main/src/rules/jsx-no-duplicate-props/readme.md',
    },
    schema: [
      {
        type: 'object',
  ...
  create(context) {
    return {
      JSXOpeningElement(node) {
        ...
        const checkPropName = (name: string, node: T.Node) => {
          ...
          if (props.has(name)) {
            context.report({
              node,
              messageId:
                name === 'class' ? 'noDuplicateClass' : 'noDuplicateProps',
            });
          }
          props.add(name);
        };
        ...
        const hasChildren =
          (node.parent as T.JSXElement | T.JSXFragment).children.length > 0;
